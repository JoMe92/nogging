...
1. Run `scripts/nogg discoveries`.
...
2. **Acknowledge** — no spec change is needed and it should stop being
   ...
   ```bash
   scripts/nogg discoveries --ack <bead-id> [<bead-id>...]
   ```

   This writes the acknowledgement ledger (`.nogging/state/acknowledged-discoveries.json`)
   ...
   `scripts/nogg` without the ledger), degrade to *leave pending* and
...
- Do not acquire the planning lock — never run `scripts/nogg plan-begin`.
...
- The only state this command may write is the acknowledgement ledger, via
  `scripts/nogg discoveries --ack` (see the per-discovery prompt).
