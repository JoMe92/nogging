...
- [ ] TASK-ASR-001 Verify `JoMe92/agentsembli-specforge` is empty and private, push only the allowlisted sanitized heads/tags from the clean mirror, record GitHub's public-only Private Vulnerability Reporting boundary for the owner cutover, then fresh-mirror-clone and prove zero legacy PR/Dolt refs, forbidden paths, credential signatures, private/local identities and maintainer paths while the repository remains private.
...
- [ ] TASK-ASR-005 Add an exact owner-run GitHub cutover checklist covering description/topics, default branch, Issues, the private-phase `SECURITY.md` route, releases, local remotes, old-repository archive/delete, successor visibility, immediate post-public Private Vulnerability Reporting enablement/verification and rollback; verify no automated command changes visibility, signs acceptance or republishes dirty backup/legacy refs.
