...
- [ ] TASK-PUB-004 Add `CONTRIBUTING.md`, `CODE_OF_CONDUCT.md`, `SECURITY.md`, support guidance, issue forms, a pull-request template, and documented GitHub description/topics/settings; verify each contributor and disclosure path is internally consistent and link-valid without changing repository visibility.
- [ ] TASK-PUB-005 Rewrite `README.md` as a user-first landing page with maturity status, audience, supported agents/platforms, safety warning, architecture summary, a copyable five-minute pinned-tag journey, and a documentation index; retain SpecForge as the standalone public project name, identify Agentsembli only as a provisional ecosystem working name, mention Agent Console only as an optional sibling, and add a visible direct-link acknowledgement that Gas Town is the conceptual inspiration while SpecForge is independently implemented, unaffiliated, and currently adopts only Beads; verify the example succeeds in a fresh repository without a product website, renamed repository, Agent Console installation, or maintainer-local path.
- [ ] TASK-PUB-006 Reconcile installation and operations documentation with current behavior: automatic Beads initialization and `--no-beads`, all Claude/Codex/Pi payloads, OpenSpec and runtime prerequisites, slugged systemd services, hook collision, pinned update, rollback, uninstall, and non-systemd operation; verify documented commands against CLI help and a fresh install.
- [ ] TASK-PUB-007 Write a public security/threat-model guide covering installed hooks and instructions, background services, network and filesystem authority, command floors, credentials, trust dialogs, restricted/trusted/orchestrator modes, vendor-specific residual risk, and safe disable/removal; verify README and `SECURITY.md` route users to it before full-access commands.

## 3. Distribution and usability

- [ ] TASK-PUB-008 Make the first public distribution explicitly GitHub-only by preventing accidental npm publication and documenting the occupied unscoped npm name; complete package repository/homepage/bugs/author/keywords metadata and verify `npm publish --dry-run` cannot publish while tagged `npx github:` installation still works.
...
- [ ] TASK-PUB-015 Generate a final public-readiness report proving TASK-PUB-001 through TASK-PUB-014, all CI gates, zero unresolved publication blockers, reviewed pending discoveries, signed acceptance, clean package contents, current release evidence, and the required SpecForge/Agentsembli/Gas Town/Beads identity statements; include an owner-only GitHub visibility checklist and verify the repository is still private when this task closes—do not change visibility or automate that action.
