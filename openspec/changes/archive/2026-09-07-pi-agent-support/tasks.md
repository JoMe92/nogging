...
- [x] TASK-PIA-007 Ship `.pi/prompts/{plan,discovery-review,sync-now}.md` carrying the same persona and step text as `.claude/commands/` and `.codex/prompts/`. Add `.pi/prompts` and `.pi/extensions` to `bin/lib/manifest.js` `verbatimDirs` and to `package.json` `files`.
- [x] TASK-PIA-008 Restructure `AGENTS.md` *Tool notes* to add a **Pi** subsection: write boundary = the guard extension (not a hook, not execpolicy); specialists = out-of-process `session launch --agent pi --role specialist:<type>`; operator entry points = `.pi/prompts/`; state plainly that Pi's `restricted` level has no network or filesystem sandbox, floor-only.
