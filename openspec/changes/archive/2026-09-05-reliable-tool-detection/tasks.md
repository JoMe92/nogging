...
- [x] TASK-RTD-003 Run `scripts/test` clean on a host whose real `/etc/profile.d` resets PATH (this repo's current host qualifies); record the before/after result (failing `cxf-doctor` checks now passing) as the Bead's evidence note.
