...
- [ ] TASK-ORC-002 Ship `.specforge/launch-profiles/orchestrator.json` — `bypassPermissions`, empty `allow`/`deny`, `additionalDirectories: ["/"]`, `specforge_floor: false`, `specforge_openspec_readonly: false`; header comment: Claude-only, no `.codex.toml`/`.pi.toml`, keys honoured only for `--role orchestrator`.
