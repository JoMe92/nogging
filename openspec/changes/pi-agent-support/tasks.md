...
- [ ] TASK-PIA-002 Add a `pi` branch to `scripts/session-launch`: `cd` into `$cwd`, pass the prompt file via `--append-system-prompt`, pass `--approve` for a non-interactive supervised launch, and forward a model/provider pair when the resolved launch profile names one. Extend `session launch --agent` to accept `pi`.
