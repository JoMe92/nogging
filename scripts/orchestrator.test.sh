#!/usr/bin/env bash
# Regression checks for `scripts/specforge orchestrator ...` (TASK-ORC-012).
#
...
make_root() {
  local r="$1"
  mkdir -p "$r/.specforge/state" "$r/scripts"
  cp -r "$repo_root/.specforge/launch-profiles" "$r/.specforge/"
  cp -r "$repo_root/.specforge/launch-prompts" "$r/.specforge/"
  ...
  python3 - "$repo_root/.specforge/config.json" "$r/.specforge/config.json" <<'PY'
import json, sys
...
lock_out=$(SPECFORGE_ROOT="$root" python3 - "$repo_root/scripts/specforge" <<'PY'
import json, sys
...
[[ -f "$root/.specforge/locks/orchestrator.lock" ]] \
  && echo "ok   - run: holds the orchestrator lock while supervising" \
  || { echo "FAIL - run: lock not held"; fail=1; }
[[ "$(record "$root/.specforge/state/sessions/$name.json" floor_lifted)" == "True" ]] \
...
[[ "$(record "$root/.specforge/state/sessions/$name.json" state)" == "stopped" ]] \
  && echo "ok   - run: marks the record terminal when the session ends" \
  || { echo "FAIL - run: record state is $(record "$root/.specforge/state/sessions/$name.json" state)"; fail=1; }
...
printf '{"name":"%s","role":"orchestrator","bead_id":null,"state":"running","floor_lifted":true,"log_path":"%s"}\n' \
  "$name" "$root/.specforge/state/sessions/$name.log" >"$root/.specforge/state/sessions/$name.json"
...
for _ in $(seq 1 80); do [[ -f "$root/.specforge/locks/orchestrator.lock" ]] && break; sleep 0.1; done
sleep 0.5
